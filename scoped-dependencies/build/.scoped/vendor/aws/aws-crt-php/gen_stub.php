...
use _CKFinder_Vendor_Prefix\PhpParser\Comment\Doc as DocComment;
use _CKFinder_Vendor_Prefix\PhpParser\Node;
use _CKFinder_Vendor_Prefix\PhpParser\Node\Expr;
use _CKFinder_Vendor_Prefix\PhpParser\Node\Name;
use _CKFinder_Vendor_Prefix\PhpParser\Node\Stmt;
use _CKFinder_Vendor_Prefix\PhpParser\Node\Stmt\Class_;
use _CKFinder_Vendor_Prefix\PhpParser\PrettyPrinter\Standard;
use _CKFinder_Vendor_Prefix\PhpParser\PrettyPrinterAbstract;
...
\class_alias('_CKFinder_Vendor_Prefix\\Context', 'Context', \false);
class SimpleType
{
    /** @var string */
    public $name;
    ...
    public static function fromNode(Node $node) : SimpleType
    {
        if ($node instanceof Node\Name) {
            if ($node->toLowerString() === 'static') {
                ...
                return new SimpleType($node->toString(), \true);
            }
    ...
    public static function fromPhpDoc(string $type) : SimpleType
    {
        switch (\strtolower($type)) {
            ...
            case "self":
            case "static":
                return new SimpleType(\strtolower($type), \true);
        }
        if (\strpos($type, "[]") !== \false) {
            return new SimpleType("array", \true);
        }
        return new SimpleType($type, \false);
    }
    public static function null() : SimpleType
    {
        return new SimpleType("null", \true);
    }
    public static function void() : SimpleType
    {
        return new SimpleType("void", \true);
    }
...
\class_alias('_CKFinder_Vendor_Prefix\\SimpleType', 'SimpleType', \false);
class Type
{
    ...
    public static function fromNode(Node $node) : Type
    {
        ...
        if ($node instanceof Node\NullableType) {
            return new Type([SimpleType::fromNode($node->type), SimpleType::null()]);
        }
...
\class_alias('_CKFinder_Vendor_Prefix\\Type', 'Type', \false);
class ArginfoType
{
...
\class_alias('_CKFinder_Vendor_Prefix\\ArginfoType', 'ArginfoType', \false);
class ArgInfo
{
    const SEND_BY_VAL = 0;
    const SEND_BY_REF = 1;
    ...
    {
        return $this->name === $other->name && $this->sendBy === $other->sendBy && $this->isVariadic === $other->isVariadic && Type::equals($this->type, $other->type) && $this->defaultValue === $other->defaultValue;
    }
...
\class_alias('_CKFinder_Vendor_Prefix\\ArgInfo', 'ArgInfo', \false);
...
\class_alias('_CKFinder_Vendor_Prefix\\FunctionOrMethodName', 'FunctionOrMethodName', \false);
...
\class_alias('_CKFinder_Vendor_Prefix\\FunctionName', 'FunctionName', \false);
...
\class_alias('_CKFinder_Vendor_Prefix\\MethodName', 'MethodName', \false);
class ReturnInfo
{
    /** @var bool */
    public $byRef;
    ...
    {
        return $this->byRef === $other->byRef && Type::equals($this->type, $other->type);
    }
    public function getMethodSynopsisType() : ?Type
    {
...
\class_alias('_CKFinder_Vendor_Prefix\\ReturnInfo', 'ReturnInfo', \false);
class FuncInfo
{
    ...
    /** @var string|null */
    public $cond;
    public function __construct(FunctionOrMethodName $name, int $classFlags, int $flags, ?string $aliasType, ?FunctionOrMethodName $alias, bool $isDeprecated, bool $verify, array $args, ReturnInfo $return, int $numRequiredArgs, ?string $cond)
    {
        $this->name = $name;
...
\class_alias('_CKFinder_Vendor_Prefix\\FuncInfo', 'FuncInfo', \false);
class ClassInfo
{
    /** @var Name */
    public $name;
...
\class_alias('_CKFinder_Vendor_Prefix\\ClassInfo', 'ClassInfo', \false);
class FileInfo
{
    /** @var FuncInfo[] */
    public $funcInfos = [];
...
\class_alias('_CKFinder_Vendor_Prefix\\FileInfo', 'FileInfo', \false);
class DocCommentTag
{
    /** @var string */
    public $name;
...
\class_alias('_CKFinder_Vendor_Prefix\\DocCommentTag', 'DocCommentTag', \false);
...
function parseFunctionLike(PrettyPrinterAbstract $prettyPrinter, FunctionOrMethodName $name, int $classFlags, int $flags, Node\FunctionLike $func, ?string $cond) : FuncInfo
{
    ...
    foreach ($func->getParams() as $i => $param) {
        ...
        if ($param->default instanceof Expr\ConstFetch && $param->default->name->toLowerString() === "null" && $type && !$type->isNullable()) {
        ...
        $args[] = new ArgInfo($varName, $sendBy, $param->variadic, $type, isset($docParamTypes[$varName]) ? Type::fromPhpDoc($docParamTypes[$varName]) : null, $param->default ? $prettyPrinter->prettyPrintExpr($param->default) : null);
    ...
    $return = new ReturnInfo($func->returnsByRef(), $returnType ? Type::fromNode($returnType) : null, $docReturnType ? Type::fromPhpDoc($docReturnType) : null);
    return new FuncInfo($name, $classFlags, $flags, $aliasType, $alias, $isDeprecated, $verify, $args, $return, $numRequiredArgs, $cond);
}
...
function handleStatements(FileInfo $fileInfo, array $stmts, PrettyPrinterAbstract $prettyPrinter)
{
    ...
    foreach ($stmts as $stmt) {
        ...
        if ($stmt instanceof Stmt\Function_) {
            $fileInfo->funcInfos[] = parseFunctionLike($prettyPrinter, new FunctionName($stmt->namespacedName), 0, 0, $stmt, $cond);
            continue;
        }
        if ($stmt instanceof Stmt\ClassLike) {
            ...
            foreach ($stmt->stmts as $classStmt) {
                ...
                $methodInfos[] = parseFunctionLike($prettyPrinter, new MethodName($className, $classStmt->name->toString()), $classFlags, $flags, $classStmt, $cond);
            }
...
function generateArgInfoCode(FileInfo $fileInfo, string $stubHash, bool $minimal) : string
{
    ...
    if ($fileInfo->generateFunctionEntries) {
        ...
        $code .= generateCodeWithConditions($fileInfo->getAllFuncInfos(), "", function (FuncInfo $funcInfo) use($fileInfo, &$generatedFunctionDeclarations) {
...
if ($verify) {
    ...
    foreach ($aliasMap as $aliasFunc) {
        ...
        \array_map(function (?ArgInfo $aliasArg, ?ArgInfo $aliasedArg) use($aliasFunc, $aliasedFunc, &$errors) {
            if ($aliasArg === null) {
