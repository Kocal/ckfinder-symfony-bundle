...
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\SignatureType;
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\Signing;
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\SigningAlgorithm;
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\SigningConfigAWS;
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\StaticCredentialsProvider;
use _CKFinder_Vendor_Prefix\AWS\CRT\HTTP\Request;
use _CKFinder_Vendor_Prefix\AWS\CRT\Auth\Signable;
...
class S3SignatureV4 extends SignatureV4
{
    ...
    private function signWithV4a(CredentialsInterface $credentials, RequestInterface $request, $signingService)
    {
        if (!\extension_loaded('awscrt')) {
            throw new CommonRuntimeException("AWS Common Runtime for PHP is required to use Signature V4A and multi-region" . " access points.  Please install it using the instructions found at" . " https://github.com/aws/aws-sdk-php/blob/master/CRT_INSTRUCTIONS.md");
        }
